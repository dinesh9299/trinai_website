...
const Home = () => {
  ...
  const [productCarouselData, setProductCarouselData] = useState([]);
  const [businessSolutions, setBusinessSolutions] = useState([]);
  const [velocitySeries, setVelocitySeries] = useState(null);
    ...
    const fetchHomeData = async () => {
      try {
        const res = await fetch(
          "http://localhost:1337/api/homehero?populate[Products][populate][productcards][populate]=image&populate[TrinaiBusinessSolutionscards][populate]=BusinessSolutionscards&populate[VelocitySeries][populate]=velocity_image&populate[smartdata][populate]=smart_ip_image",
        );
        ...
        if (data?.Products?.productcards) {
          const products = data.Products.productcards.map((item) => ({
            name: item.title,
            image: "http://localhost:1337" + item.image.url,
          }));

          setProductCarouselData(products);
        }

        /* ---------------- BUSINESS SOLUTIONS ---------------- */

        if (data?.TrinaiBusinessSolutionscards?.BusinessSolutionscards) {
          const solutions =
            data.TrinaiBusinessSolutionscards.BusinessSolutionscards.map(
              (item) => ({
                title: item.title,
        ...
        if (data?.VelocitySeries) {
          setVelocitySeries({
            title: data.VelocitySeries.title,
            description: data.VelocitySeries.description,
            image:
              "http://localhost:1337" + data.VelocitySeries.velocity_image.url,
          });
        }
        ...
        if (data?.smartdata) {
          setSmartData({
            title: data.smartdata.title,
            description: data.smartdata.description,
            image: "http://localhost:1337" + data.smartdata.smart_ip_image.url,
          });
        }
  ...
  return (
    <div className="w-full bg-white">
      ...
      <div className="relative w-full h-[430px] sm:h-[530px] lg:h-[700px] overflow-hidden">
        ...
        <div className="relative z-20 h-full flex flex-col justify-center text-white px-4 sm:px-8 md:px-14 lg:px-28">
          ...
          <motion.p
            ...
            className="text-sm sm:text-base md:text-lg lg:text-2xl mt-4"
          >
            Delivering end-to-end intelligent surveillance solutions for safer
            spaces.
          </motion.p>
      ...
      {velocitySeries && (
        <div className="p-5">
          <div className="lg:grid rounded-md grid-cols-2 p-8 shadow border">
            <img src={velocitySeries.image} alt="Velocity Series" />

            <div className="m-2">
              <div className="text-2xl font-bold mb-1 bg-gradient-to-r from-[#00ADE7] to-[#305292] bg-clip-text text-transparent">
                {velocitySeries.title}
              </div>

              <p className="text-gray-600 text-lg max-w-xl">
                {velocitySeries.description}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="lg:p-10 bg-slate-50">
          <div className="lg:grid grid-cols-2 p-8">
            <div>
              ...
              <p className="text-gray-700 text-xl">{smartData.description}</p>
      ...
      <ScrollTop
        target="window"
        threshold={100}
        className="w-12 h-12 rounded-full bg-gradient-to-r from-[#00ADE7] to-[#305292] flex items-center justify-center shadow-lg"
      />
    </div>
  );
};
