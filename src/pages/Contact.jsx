...
const Contact = () => {
  ...
  // 🔄 Config
  const TURNSTILE_CONFIG = {
    siteKey: "0x4AAAAAACuHkkNabbbUTjv3",
  };

  // 🔄 Strapi API URL (uses env var or fallback)
  const STRAPI_URL =
    import.meta.env?.VITE_STRAPI_URL || "http://localhost:1337";
  // 🔄 Backend API for form submission
  const API_URL = import.meta.env?.VITE_API_URL || "http://localhost:3001";

  // 🔄 Icon Mapping for Dynamic Social Icons (FIXED: No require(), all imported above)
  const getIconComponent = (iconName) => {
    ...
    return icons[iconName] || FaFacebookF; // Fallback icon
  };
  ...
  useEffect(() => {
    ...
    const fetchContactData = async () => {
      try {
        console.log("🔄 Fetching Contact Page from Strapi...");

        const response = await fetch(
          `${STRAPI_URL}/api/contact-pages?populate=*`,
        );
        ...
        console.log("✅ Strapi Contact Response:", json);
        ...
        if (data && isMounted) {
          ...
          console.log("✅ Contact Data Loaded:", {
            ...
            social: data.social_links?.length,
          });
        } else if (isMounted) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <div className="text-center">
          <div className="w-16 h-16 border-4 border-[#00ADE7] border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
      ...
      <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4">
        <div className="text-center max-w-md">
          ...
          <button
            ...
            className="bg-gradient-to-r from-[#00ADE7] to-[#305292] text-white py-2 px-6 rounded-lg hover:shadow-lg transition"
          >
            Try Again
          </button>
        </div>
  ...
  const bannerTitle = contactData?.banner_title || "Contact Our Team";
  const bannerDescription =
    contactData?.banner_description ||
    "Contact Trinai today for customized, high-quality security and surveillance solutions.";
  const bannerImage = contactData?.banner_image?.url;

  const formTitle = contactData?.form_title || "Send us a Message";
  const formSubtitle =
    contactData?.form_subtitle ||
    "Please let us know if you have a question or want to leave a comment.";

  const contactEmail = contactData?.contact_email || "trinai@brihaspathi.com";
  const contactPhone = contactData?.contact_phone || "+91 9885888835";
  const emailLabel = contactData?.contact_email_label || "Email Us";
  const phoneLabel = contactData?.contact_phone_label || "Call Us";

  const addresses = contactData?.addresses || [];
  const socialLinks =
    contactData?.social_links?.filter((link) => link.is_active) || [];
  const mapEmbedUrl = contactData?.map_embed_url;
  const followUsLabel = socialLinks[0]?.fallowus || "Follow Us";
    ...
    <div className="relative">
      ...
      <div className="relative h-[70vh] w-full overflow-hidden">
        {bannerImage ? (
          <div
            className="absolute inset-0 bg-fixed bg-center bg-cover"
            style={{ backgroundImage: `url('${STRAPI_URL}${bannerImage}')` }}
          >
            <img
              src={`${STRAPI_URL}${bannerImage}`}
              alt={bannerTitle}
              className="hidden"
              onError={(e) => {
                e.target.onerror = null;
          ...
          <div className="banner-fallback absolute inset-0 bg-gradient-to-r from-[#00ADE7] to-[#305292]" />
        )}
        <div className="relative z-10 flex flex-col items-center justify-center h-full text-center px-6 font-sans">
          <h1 className="text-white text-3xl md:text-5xl font-bold mb-4 drop-shadow-lg">
            {bannerTitle}
      ...
      <div className="bg-slate-50 py-12 px-4 lg:px-8 font-sans">
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 items-start">
              ...
              <div className="my-4">
                ...
                <div className="w-full min-h-24 flex items-center justify-center bg-slate-50 border border-slate-200 rounded-lg p-2">
                  <Turnstile
                    ...
                    onError={handleTurnstileError}
                    onExpired={handleTurnstileExpired}
                    options={{ size: "normal", theme: "light", language: "en" }}
                  />
                </div>
              ...
              <div className="flex justify-end pt-2">
                {loader ? (
                  ...
                  <button
                    type="submit"
                    disabled={!turnstileToken || turnstileToken.length < 10}
                    className={`py-3 px-10 rounded-lg transition-all duration-300 shadow-md font-sans font-medium ${
                      !turnstileToken || turnstileToken.length < 10
                        ? "bg-gray-300 text-gray-500 cursor-not-allowed"
                        : "bg-gradient-to-r from-[#00ADE7] to-[#305292] text-white hover:from-[#305292] hover:to-[#00ADE7] hover:shadow-lg"
                    }`}
          ...
          {/* 🔄 Right Side Cards - Dynamic from Strapi */}
          <div className="space-y-6">
            {/* Address Cards - Dynamic */}
            <div className="bg-white rounded-2xl shadow-md p-6 border border-slate-100 hover:shadow-lg transition-shadow">
              <h3 className="text-xl font-bold bg-gradient-to-r from-[#00ADE7] to-[#305292] bg-clip-text text-transparent mb-4 flex items-center gap-2">
                <FaLocationDot className="gradient-icon" size={20} />
                {addresses[0]?.location || "Our Locations"}
              </h3>
              <div className="space-y-4">
                {addresses.length > 0 ? (
                  addresses.map((addr, index) => (
                    <div key={addr.id || index}>
                      <p className="font-semibold text-gray-700">
                        {addr.location_type}
                      </p>
                      <p className="text-gray-600 text-sm mt-1 pl-6">
                      ...
                      {addr.map_link && (
                        <iframe
                          src={addr.map_link}
                          className="w-full h-48 mt-3 rounded-lg border-0"
                          loading="lazy"
                          title={`${addr.location_type} Map`}
                        />
                      )}
                    </div>
                  ))
            ...
            {/* 🔄 Social Links - Dynamic from Strapi (FIXED ICON MAPPING) */}
            <div className="bg-white rounded-2xl shadow-md p-6 border border-slate-100 hover:shadow-lg transition-shadow">
              <h3 className="text-xl font-bold bg-gradient-to-r from-[#00ADE7] to-[#305292] bg-clip-text text-transparent mb-4">
                {followUsLabel}
              </h3>
